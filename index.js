        ...
        let txt = body.normalize("NFD")
            ...
            if (num === from || num === '0'){
                if(txt === 'hola' || txt === 'ola' || txt === 'buenos dias' || txt === 'buenas tardes' || txt === 'buenas noches' || txt === 'buen dia'){
                    ...
                    sendMessage(from, 'Para hacer un reporte sobre una falla en un equipo envía los siguientes datos anteponiendo un guión alto *-* antes de los datos (por ejemplo "-4001", "-Juan perez", etc.) despues de cada dato recibirás un mensaje de confiramación: \n \n *Nombre* \n *Laboratio* \n *Numero del equipo*  \n *Materia* \n *Descripción del reporte* \n \n Una vez guardados los datos escibe *3* para guardar el reporte. \n \n En caso de que te quivoques con algún dato escribe la palabra *del* para borrar los datos.');
                }else if (txt === '2'){
                    sendMessage(from, 'Para hacer un reporte sobre una incidencia administrativa en un equipo envía los siguientes datos anteponiendo un punto *.* antes de los datos (por ejemplo ".4001", ".Juan perez", etc.) despues de cada dato recibirás un mensaje de confiramación: \n \n *Nombre* \n *Número de laboratio* \n *Materia* \n *Descripción del reporte* \n \n Una vez guardados los datos escibe *4* para guardar el reporte. \n \n En caso de que te quivoques con algún dato escribe la palabra *del* para borrar los datos.');
                }else if (txt.charAt(0)==='-'){
                    datosE.push(txt.slice(1,250));
                    sendMessage(from, 'Dato recibido');
                    console.log(datosE);
                }else if (txt.charAt(0)==='.'){
                    datosA.push(txt.slice(1,250));
                    sendMessage(from, 'Dato recibido');
                    console.log(datosA);
                }else if(txt === '3'){
                    insertHw(datosE);
                    ...
                    num = '0';
                    sendMessage(from, 'El reporte de ha guarado con exito. \n Hasta luego😀');
                }else if(txt === '4'){
                    insertAd(datosA);
